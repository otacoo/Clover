...
import org.otacoo.chan.core.database.DatabaseBoardManager;
...
public class BoardRepository implements Observer {
    ...
    private void updateWith(List<Pair<Site, List<Board>>> databaseData) {
        ...
        for (Pair<Site, List<Board>> item : databaseData) {
            ...
            List<Board> savedBoardsList = new ArrayList<>();
            for (Board board : item.second) {
                if (board.saved) savedBoardsList.add(board);
            }
            if (!savedBoardsList.isEmpty()) {
                saved.add(new SiteBoards(item.first, savedBoardsList));
            }
        }
