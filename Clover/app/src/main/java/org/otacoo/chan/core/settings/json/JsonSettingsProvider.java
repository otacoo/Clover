...
public class JsonSettingsProvider implements SettingProvider {
    ...
    public String getString(String key, String def) {
        ...
        String result;
        if (setting != null) {
            result = ((StringJsonSetting) setting).value;
        } else {
            result = def;
        }
        if (key.equals("preference_captcha_type")) {
            Logger.test("JsonSettingsProvider.getString(preference_captcha_type): found=" + (setting != null) + ", result=" + result + ", def=" + def);
        }
        return result;
    }
